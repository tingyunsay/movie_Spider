...
import hashlib
from movie_Spider.Path_translate import Relative_to_Absolute,Relative_to_Absolute2,Get_Valid_Url,get_HeadUrl
from movie_Spider.Total_page_circulate import Total_page_circulate,Turn_True_Page,Total_Page_Byyourself
...
class movieSpider(scrapy.Spider):
	...
	def parse_json(self,response):
		...
		depth = 0
		if isinstance(Max_Page['index'],list):
				try:
		...
		elif isinstance(Max_Page['index'],int):
				max_pages = Total_Page_Byyourself(self.name)
		else:
	...
	def parse_json2(self,response):
		...
		for i in res_json:
				#对乐视做了特化，这里是一堆unicode的vid，取第一个，它们是以，分隔的，直接\d+就好
				if re.search('\d+',i.get(All_Detail_Page['index'][length-1])):
						detail_url.append(re.search('\d+',i.get(All_Detail_Page['index'][length-1])).group())
